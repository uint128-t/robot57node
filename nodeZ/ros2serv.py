...
class MyNode(Node):
    def __init__(self):
        ...
        self.subscription_plan = self.create_subscription(Path,'/plan',self.listener_callback_map,qos_profile)

        self.get_logger().info('Node1 started!')

    def listener_callback_map(self,msg):
        self.get_logger().info('Received path with %d poses' % len(msg.poses))
        poses = []
...
def main(args=None):
    ...
    node = MyNode()
    twistpublisher = rclpy.create_node("webcontrol").create_publisher(Twist,"cmd_vel",qos_profile)
